        ...
        <form class="geniusform" action="{{route('admin-ps-update')}}" method="POST">
            ...
            <div class="form-row">
                <div class="form-group ml-5 col-md-5 ">
                    ...
                    <input type="text" name="blog_subtitle"  required  value="{{$ps->blog_subtitle}}" class="form-control" id="stitle">
                </div>
                <div class="form-group ml-5  col-md-5">
                    <label for="title">{{__("Title")}}</label>
                    <input type="text" name="blog_title" required  class="form-control" value="{{$ps->blog_title}}" id="title" >
                </div>
                <div class="form-group mx-auto">
                    <label for="details">{{ __('Text ') }}</label>
                    <textarea class="form-control summernote"  id="details" name="blog_text" required rows="3" placeholder="{{__('Enter Text')}}">{{ $ps->blog_text }}</textarea>
                </div>
            </div>
            <button type="submit" id="submit-btn" class="btn btn-primary mx-auto d-block mb-2">{{ __('Submit') }}</button>
        </form>
      </div>
    </div>
